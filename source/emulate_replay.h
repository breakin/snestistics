...
struct EmulateReplay {
	EmulateReplay(const std::string &trace_file_name) {
		_trace_file = fopen(trace_file_name.c_str(), "rb");
		#ifdef VERIFY_OPS
		{
			StringBuilder sb;
			sb.add(trace_file_name);
			sb.add("_helper");
			_trace_helper = fopen(sb.c_str(), "rb"); // NOTE: We handle this failing
		}
		#endif
		read_next_event();
	}
	...
	bool skip_until_nmi(const Options &options, STATE &regs, const uint32_t target_skip_nmi) {
		...
		do {
			FILE *f = fopen(options.trace_file_skip_cache(0).c_str(), "rb");
			if (!f)
				break;
	...
	uint64_t _last_after_nmi_offset = 0;

	template<typename STATE>
	bool next(STATE &regs) {
		...
		const uint32_t PC_before_op = regs._PC;
			...
			if (_trace_helper && do_event != Events::RESET) {
				check_diff(h.registers_before, h.registers_before, h.registers_after, regs, P_before_op, "BEFORE");
			}
		#endif
		...
		#ifdef VERIFY_OPS
			if (_trace_helper) {
				check_diff(h.registers_after, h.registers_before, h.registers_after, regs, P_before_op, "AFTER");
			}
		#endif
		_current_op++;
		return true;
...
private:
	...
	#ifdef VERIFY_OPS
		void check_diff(const TraceRegisters &test, const TraceRegisters &b, const TraceRegisters &a, const EmulateRegisters &regs, uint16_t P_before_op, const char * const str) const {
			// b=before, a=after
			bool has_diff = 
